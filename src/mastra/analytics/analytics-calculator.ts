...
export function calculatePerformanceStats(
  ...
  // Score distribution
  const excellent = sessions.filter((s) => (s.adviceQualityScore || 0) >= 9)
    .length;
  const good = sessions.filter(
    (s) =>
      (s.adviceQualityScore || 0) >= 7 && (s.adviceQualityScore || 0) < 9
  ).length;
  const average = sessions.filter(
    (s) =>
      (s.adviceQualityScore || 0) >= 5 && (s.adviceQualityScore || 0) < 7
  ).length;
  ...
  // Evaluation rates
  const empathyRate =
    (sessions.filter((s) => s.evaluation?.wasEmpathetic).length /
      totalSessions) *
    100;
  const actionabilityRate =
    ...
    100;
  const accuracyRate =
    (sessions.filter((s) => s.evaluation?.wasAccurate).length /
      totalSessions) *
    100;
...
export function calculateTopicExpertise(
  ...
  const topicData: TopicExpertiseData[] = allTopics.map((topic) => {
    ...
    const avgQuality =
      sessionCount > 0
        ? topicSessions.reduce((sum, s) => sum + (s.adviceQualityScore || 0), 0) /
          sessionCount
        : 0;
    ...
    const successRate = sessionCount > 0 ? (successfulSessions / sessionCount) * 100 : 0;
...
export function calculateCharacterAnalytics(
  ...
  const characterStats: CharacterTypeStats[] = characterIds.map(
    (characterId) => {
      ...
      return {
        characterId,
        characterName: characterId.replace(/-/g, " ").replace(/\b\w/g, (l) => l.toUpperCase()),
        totalSessions,
        averageQuality: parseFloat(avgQuality.toFixed(2)),
        successRate: parseFloat(successRate.toFixed(2)),
...
export function calculateFinancialImpact(
  ...
  const coinsByQuality = qualityRanges.map(({ range, min, max }) => {
    const rangeSessions = sessions.filter(
      (s) => (s.adviceQualityScore || 0) >= min && (s.adviceQualityScore || 0) < max
    );
    const avgCoins =

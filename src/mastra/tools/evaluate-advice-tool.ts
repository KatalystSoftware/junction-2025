...
export const evaluateAdviceTool = {
  ...
  execute: async (context: {
    ...
    try {
      const { advice, scenario, characterPersonality, character, conversationHistory } = context;
...
`;
      ...
      const willFollowAdvice = evaluation.characterProgression?.willFollowAdvice ?? false;
      ...
      return {
        ...
        topicsCovered: (evaluation.topicsCovered || [scenario.topic]) as FinancialTopic[],
      ...
      const hasSteps = /\d+\.|first|then|finally|1\)|2\)/.test(advice.toLowerCase());
